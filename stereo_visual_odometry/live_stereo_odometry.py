...
class VisualOdometry():
    def __init__(self, data_dir):
        ...
        self.lk_params = dict(winSize=(15, 15), flags=cv2.MOTION_AFFINE, maxLevel=3, criteria=(
            cv2.TermCriteria_EPS | cv2.TermCriteria_COUNT, 50, 0.03))

    # This is for the case of using a dataset
    @staticmethod
    ...
    def reprojection_residuals(self, dof, q1, q2, Q1, Q2):
        """
        Reproject 3D points again in the image plane, then calculate the residuals, and try to optimize it.
    ...
    def get_tiled_keypoints(self, img, tile_h, tile_w):
        ...
        kp_list = [get_kps(x, y) for y in range(0, h, tile_h)
    ...
    def track_keypoints(self, img1, img2, kp1, max_error=4):
        ...
        trackpoints2, st, err = cv2.calcOpticalFlowPyrLK(
            img1, img2, trackpoints1, None, **self.lk_params)
        ...
        h, w = img1.shape
        in_bounds = np.where(np.logical_and(
            trackpoints2[:, 1] < h, trackpoints2[:, 0] < w), True, False)
    ...
    def estimate_pose(self, q1, q2, Q1, Q2, max_iter=100):
        ...
        for _ in range(max_iter):
            ...
            sample_q1, sample_q2, sample_Q1, sample_Q2 = q1[
                sample_idx], q2[sample_idx], Q1[sample_idx], Q2[sample_idx]
    ...
    def get_pose(self, i):
        ...
        self.disparities.append(np.divide(self.disparity.compute(
            img2_l, self.images_r[i]).astype(np.float32), 16))

        # Calculate the right keypoints
        tp1_l, tp1_r, tp2_l, tp2_r = self.calculate_right_qs(
            tp1_l, tp2_l, self.disparities[i - 1], self.disparities[i])
    ...
    def get_pose(self, old_imgL, old_imgR, new_imgL, new_imgR):
        ...
        new_disp = np.divide(self.disparity.compute(
            new_imgL, new_imgR).astype(np.float32), 16)

        # Calculate the right keypoints
        tp1_l, tp1_r, tp2_l, tp2_r = self.calculate_right_qs(
            tp1_l, tp2_l, old_disp, new_disp)
