...
def calibrate_camera(objpoints, imgpoints, frameSize):
    ...
    np.savez("single_camera_calib_params", cameraMatrix=cameraMatrix, dist=dist, rvecs=rvecs, tvecs=tvecs)
...
def calculate_reprojection_error(objpoints, imgpoints, cameraMatrix, dist, rvecs, tvecs):
    ...
    logger.info(f"Total Error: {total_error:.3f}, Mean Error: {mean_error:.3f}")
...
def main():
    ...
    cameraMatrix, dist = calibrate_camera(objpoints, imgpoints, frameSize)
    undistort_image('./single_camera_images/2023-12-09-014609.jpg', cameraMatrix, dist)
    calculate_reprojection_error(objpoints, imgpoints, cameraMatrix, dist, rvecs, tvecs)
